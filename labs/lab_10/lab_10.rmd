...
md_statewide_county <- read.csv('data/md_statewide_county.csv') |> clean_names() |> mutate(geoid = as.character(geoid))

md_statewide_county 

md_statewide_county <- md_statewide_county |>
  group_by(county) |>
  mutate(total_governor = sum(cox+moore+lashar+wallace+harding+sefcik+gov_writein)) |>
  mutate(pct_moore = (moore/total_governor)*100) |>
  mutate(total_ag = sum(peroutka+brown+ag_writein)) |>
...
md_voting_geometry <- md_counties |> inner_join(md_statewide_county, by="geoid")
...
ggplot() +
  geom_sf(data=md_voting_geometry, aes(fill=pct_diff)) +
  theme_minimal() +
...
md_county_outages <- read_csv("data/county_outages_march2023.csv") |>
  clean_names() |>
...
md_outages_geometry <- md_counties |> inner_join(md_county_outages, by=c('name'='county'))
...
electric_vehicles_geometry <- md_counties |> inner_join(electric_vehicles, by=c('name'='county'))
...
ggplot() +
  geom_sf(data=electric_vehicles_geometry, aes(fill=pct_change)) +
  theme_minimal() +
  scale_fill_viridis_b(option="magma", trans='log')
...
electric_vehicles_pop <- electric_vehicles_geometry |> 
...
ggplot() +
  geom_sf(data=electric_vehicles_pop, aes(fill=rate)) +
  theme_minimal() +
  scale_fill_viridis_b(option="magma", trans='log')

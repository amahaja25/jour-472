...
**A1. Since this bar chart is looking at the percentage of calls rather than the number of calls, August is no longer the month with the highest figures, and December is now the highest. Instead of February being the lowest, January is in this bar chart.**
...
pg_calls_by_month |>
  ...
  labs(
    title="Prince George's County Overdose Calls by Month",
    x = "month",
...
**A2. Overdose calls hit a peak at the middle of the year, and fluctuate throughout the rest of the year. The sharp drop at the end of the year might be because there is a week 53 in the data with 4 calls, which is an outlier.**
...
pg_calls_by_week |>
  ggplot() +
  geom_line(aes(x=week, y=percent)) +
  labs(
     title="Prince George's County Overdose Calls by Week",
    x = "week",
    y = "percentage of calls",
    caption = "source: Prince George's County EMS"
  )
...
**A3. Zipcodes with higher median incomes seem to have less overdose calls while zipcodes with lower median incomes see more 911 overdose calls.**
...
pg_calls_income_zip <- pg_calls_zipcode |>

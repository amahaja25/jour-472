...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer The columns have data about the zipcodes, names, shape of each geographic area, etc.**
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer Information about each zipcode in Maryland, including its shape as a map.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The code shows a map of each zipcode in Maryland through ggplot.**
...
**Task** Run the following code. Describe the output in the space below. **Answer Installs the tigris library.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The code has information about all counties in Maryland rather than zipcodes.**
...
**Task** Run the following code. Describe the output in the space below. **Answer Filters for Maryland FIPS code, which is 24.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The output is a map of all Maryland counties.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The code now shows the boundaries for both the counties and zip codes in Maryland.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The foreclosure data for March 2023 is combined with zip codes in Maryland.**
...
**Task** Run the following code. Describe the output in the space below. **Answer Now both the zip code and zip code shape are attached to the foreclosure data.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The map is color coded based on the number of foreclosures by zip code.**
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer Loaded libraries and census API key.**
...
library(janitor)
library(tidycensus)
census_api_key("62e3c67ce129c65ce2709f84982aa405a9811133")
```
...
**Task** Run the following code. Describe the output in the space below. **Answer The code created a data frame based on county foreclosure data and another data frame from the population data from the census.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The md_county_population data frame has the shape and coordinates for the borders of each county in Maryland as well as the estimate for the population.**
...
**Task** Run the following code. Describe the output in the space below. **Answer md_county_notices has infromation about the type of foreclosure, the county and number of notices.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The code mutates the county names to remove ", Maryland" as well as select for only county, population and geometry.**
...
**Task** Run the following code. Describe the output in the space below. **Answer Now the population and notices are joined by county.**
...
**Task** Run the following code. Describe the output in the space below. **Answer md_pop_with_foreclosures has a column, "rate," that shows the per capita rate of foreclosures by county.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The code creates a map with labels of the foreclosure rate by county color coded.**
...
**Task** Run the following code. Describe the output in the space below. **Answer Now the color scale does not blend together so the differences in rate are more distinct from each other.**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer The map is color coded to the exact decimal point now.**

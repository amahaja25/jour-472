...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer** Yes, the four data sets have the same number of columns and the same column names. The only difference is the ordering of the columns in some of them.
...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer** I want to know the reason behind the drop in LIB from 2018 to 2020. There's also NA for the total in 2022 for Frederick. Both Democrat and Republican registration seem to be increasing consistently, though Democrats are increasing by more.
...
**Task** Create a new dataframe with population and voter data joined together. Add a description of what this code does to your reference notebook. This combines the data by columns
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer** The Maryland demographic data contains information about the amount of people who are over 18, and the population of people by race, including if they are one race or mixed race.
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** The county with the highest 18+ Black population is Prince George's county, which is 77.1% Democrat.
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** Howard County has the highest 18+ Asian population out of all Maryland counties, and has a 52.6% Democrat population. This figure is much smaller than the last demographic question, as the Asian population's percentage is much smaller across the board.
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer Now, it is a \<chr\> or character.**

...
```{r}
voters_by_county <- read_csv("data/tabula-EligibleActiveVotersbyCounty-GG22.csv")
voters_by_county
```
...
**Task** Export the CSV file to your pre_lab_05 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** The top row is headers when it should not be.
...
**Task** Load the data without headers by running the codeblock below. What problems remain? **Answer** The headers are unlabeled so we don't know what we are looking at.
...
**Task** Run the codeblock below. What else do we need to do? **Answer** We need to get rid of the periods in the jurisdiction names.

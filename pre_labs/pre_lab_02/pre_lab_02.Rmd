...
**Answer** A new variable is important so that the original data is still available to use when needed while we can look at specific aspects of the data with the variable.
...
**Answer:** The expenditure date went from a <chr> to a <date> category.
...
**Answer** The month column lists the values as a date on the first of each month, and the total expenditures are grouped by month.
...
**Answer** The code creates columns total_votes and pct_moore, which refer to the total votes and percentage of votes for Moore respectively. Both columns are based on general_22, which is just the whole dataset.  
...
**Answer** The answer changed from the previous task as it was arranged in descending order this time by the percent of votes for Moore.
...
**Answer** Yes, this is an interesting answer, as it shows the counties with the least percentage of votes for Moore first. 
...
**Answer** str_to_lower converts a string to lower case and str_to_title capitalizes the first letter of every word.
...
**Answer** The mutate statement modifies the state column and gives it an abbreviation for the state.
...
**Answer**  Yes, this answer makes sense as the new maryland_expenses_with_state is sorted by state and then arranged from highest total to lowest, just as prompted in the code.
...
1.  What department has the most classes that mention "Ethics?"

2.  What city in Maryland had the highest total expenditures?

3.  Which instructor taught the most classes in 2023?

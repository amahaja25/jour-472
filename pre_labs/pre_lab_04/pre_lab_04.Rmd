...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer Some of the Grantee names are counted as two separate entries even if it is the same grantee due to discrepancies in the names.**
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer Refinr changed Washington County Museum of Fine Arts to all uppercase and changed Charles County Arts Alliance Inc to CHARLES COUNTY ARTS ALLIANCE.**
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer refinr turned Washington Adventist University into all capitalized, and turned** **WASHINGTON CO COMM. ACTION COUNCIL INC into just WASHINGTON COMM ACTION COUNCIL.**
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer n_gram_merge did make some of the same changes as refinr, like with WASHINGTON COMM ACTION COUNCIL, but did not change some of the same rows. For example, n_gram_merge turned WASHINGTON ADVENTIST UNIVERSITY to Washington Adventist University.**

...
<https://datawrapper.dwcdn.net/TYqAM/2/>
...
md_gov_county <- md_gov_county |>
  group_by(county) |>
  mutate(total_votes = sum(cox + moore + lashar + wallace + harding + write_ins)) |>
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. **Answer** Describe the changes from the previous map **The map now has only Maryland counties rather than the entire United States.**
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "0.0%")}} **Answer** Describe the changes from the previous map **Now the map's tool tips shows percentages with a percentage sign and only one decimal place.**
...
<https://datawrapper.dwcdn.net/U76CR/1/>

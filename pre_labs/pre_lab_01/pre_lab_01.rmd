...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer: chr/character**
...
**Answer: First line selects the umd_courses dataset. Second line groups the data within dataset by term. Third line then summarizes the count of classes based on the categories for term.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: Now the result is arranged in descending order.**
...
**Answer: Rather than displaying just the count of classes by each term, the output now sorts the dataset by term and department because instead of arranging by term and count_classes, umd_courses is arranged by term and description in the sixth line of code.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: The data is divided into the total seats, number of classes and term due to it being arranged in descending order by total_seats and summarized by count_classes and total_seats.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer: The data is sorted in descending order by total_seats like it was previously but now there are categories displaying the mean and median number of seats per class for each term.**
...
**Answer: The data is still sorted by descending order of seats but now is grouped by department rather than term.**
...
1.  Which departments have the highest and lowest number of total seats?

2.  Are there any courses that are significant outliers in terms of the mean number of seats?

3.  What is the trend for the number of total seats by term over time?

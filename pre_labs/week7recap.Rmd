...
**A1:** The file name comes from a url rather than from a folder/downloaded data set.
...
**A2:** There are 9806 rows of data. Based on the columns, we can gather information about the location and type of earthquakes as well as the magnitude.
...
**A3:** The deepest earthquake's depth was 669.9820 and the magnitude was 4.20.
...
**A4:** 12 earthquakes had a magnitude over 6.
...
**A5: 5 earthquakes in the dataset fit both criteria.**
...
**A6: 7439 rows in the dataset fit either criteria.**
...
**A7:** 3442 earthquakes took place in Alaska.
...
earthquakes |>
  group_by(type) |>
  summarise(total_earthquakes = n()) |>
  arrange(desc(total_earthquakes))

```

**A8:** The first two most common types of tremors in this data set are earthquakes, as there were 9568, and quarry blasts, which there were 130 of.
...
**A9:** The average depth of earthquakes is 25.275. The average depth for ice quakes is 0, which seems unusual.
...
**A10:** Using mutate, I can create a new column, time, that only has the hour.
...
earthquakes <- earthquakes |>
  mutate(serious = case_when(mag > 3 ~ 'serious', .default = 'not serious'))

```

**A11:** Using mutate, I created a new column that labels a given event as serious if the magnitude is greater than 3 and not serious if the magnitude is not greater than 3.
...
earthquakes |> 
  ...
  group_by(hour) |>
  summarise(serious_perhour = n()) |>
  arrange(serious_perhour)

```

**A12**: First, I filtered just for the serious earthquakes, then grouped them by the hour in which they occurred. Then I summarized the number of serious earthquakes per hour. The least serious earthquakes happened at 12 p.m. and the most in the hour of 2 a.m.

...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer Now it does not show parsing errors as all 373564 rows were taken in account using guess_max.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer \<chr\>**
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think is happening with this data? **Answer 550 zip codes do not have five characters. This data will probably not be used as it is wrong and unhelpful, as we have no idea what the actual zip codes are. Further reporting could also be used to get updated and correct information about these zip codes.**
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer Some categories are NA, so they're neither a grant or loan, but we have no idea what the payments were for then.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer The smallest amount is** \$60 which is much smaller than all the other data.
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer Some of the headers were combined to reflect the grantors that were the same.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer 25 duplicates, meaning there are 50 rows of duplicates.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer zip5 now only had five digit zip codes, as the nine digit codes have been converted.**
